...
# ------------------------------------------------------------------------------------------
slope_break <- function(WSEw, exposure, win = 4, thres = 0.015)
{
  # Performs slope break method on a channel
  # Returns null if slope break is not applicable
  # Otherwise, returns location of slope break and estimated depth
  
  # Remove unobserved channel geometry
  if (exposure<1)
  {
    unobserved.ind <- which(WSEw$w/wbf < (1-exposure))
    # print(WSEw)
    WSEw <- WSEw[-unobserved.ind,]
  }
  
  sb.ind <- test_slope_break(WSEw$WSE, WSEw$w, thres = thres, window = win)
  if (!is.null(sb.ind))
  {
    # If location is optimal, estimate z0
    sb.location <- TRUE
  ...
  sb.result <- list(sb.ind, z0)
  names(sb.result) <- c("sb.ind","z0")
  return(sb.result)
...
calc_WSEw <- function(interval = 0.05, dx = 1, geometry_data)
{
  ...
  # geometry_data = file name for output from auto_transects
  #
  # OUTPUTS

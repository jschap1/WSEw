...
# Load gauge data
stage_name <- "/Users/jschap/Documents/Research/SWOTBATH/Data/USACE/Stage/miss_riv_at_lock_and_dam_21_2000-2018.txt"
gauge <- read.table(stage_name, header = TRUE, stringsAsFactors = FALSE)
h <- as.numeric(gauge$Height)

#' Perform distribution fits
#' 
#' Fits distributions and makes a plot of results
#' @param dists list of names of distributions to fit
#' @examples
stage_name <- "/Users/jschap/Documents/Research/SWOTBATH/Data/USACE/Stage/miss_riv_at_lock_and_dam_21_2000-2018.txt"
gauge <- read.table(stage_name, header = TRUE, stringsAsFactors = FALSE)
h <- as.numeric(gauge$Height)
stage_name <- "/Users/jschap/Documents/Research/SWOTBATH/Data/USACE/Stage/miss_at_quincy_1947-2018.txt"
gauge <- read.table(stage_name, header = FALSE, stringsAsFactors = FALSE)
h <- as.numeric(gauge$V4)
h <- h[-which(is.na(h))]
plot.title <- "Lock and Dam 21"
plot.title <- "Miss. Riv. at Quincy"
hist(h, prob = TRUE, "fd", main = plot.title, 
     xlab = "Stage (ft)", xlim = c(460,490))
plot(density(h, na.rm = TRUE), 
     main = "Miss. Riv. at Quincy", 
     xlab = "Stage (ft)", lwd = 2)
names <- c("weibull")
names <- c("gamma","weibull","lognormal","exponential","burr")
perform_dist_fits(h, names)
legend("topright", 
       legend = c("empirical","gamma", "weibull", "lognormal", "exponential", "burr"), 
       fill = c("black","red","green", "yellow", "blue", "darkgreen"))

perform_dist_fits <- function(x, names)
{
  ...
  for(name in names)
  {
    ...
    if (name == "gamma")
    {
      ...
      lines(x = seq(min(x),max(x),length.out = 500), y.gamma, col = "red")
    ...
    {
      print("fitting burr")
      fit.params <- fitdist(x, "burr", start = c(scale = 460, shape1 = 2349, shape2 = 0.035))
      y.burr <- dburr(seq(min(x),max(x),length.out = 500), 
                      shape1 = as.numeric(fit.params$estimate[2]),
                      shape2 = as.numeric(fit.params$estimate[3]),
                      scale = as.numeric(fit.params$estimate[1]))
      lines(x = seq(min(x), max(x), length.out = 500), y.burr, col = "darkgreen")
    }

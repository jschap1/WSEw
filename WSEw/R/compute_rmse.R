...
compute_rmse <- function(pred_vals, true_vals)
{
  ...
  for (k in 1:n_levels)
  {
    ...
    rmse[k,3] <- ((1/length(true_vals))*t(nl[,k] - true_vals)%*%(nl[,k] - true_vals))^0.5
    rmse[k,4] <- ((1/length(true_vals))*t(nlsb[,k] - true_vals)%*%(nlsb[,k] - true_vals))^0.5
  }
  rmse <- as.data.frame(rmse)
  names(rmse) <- c("l","sb","nl","nlsb")
